( function( $, undefined ) {
	...
	$.GridGallery.defaults 	= {
		...
		fadeSpeed	: 1000, // transition speed for grid lines
        fadeSpeed2	: 500 // transition speed for navigation
	};
		...
		_addNavigation		: function() {
			...
			if( this.slidesCount > 1 ) {
				...
				for( var i = 0; i < this.slidesCount; ++i ) {
					...
					this.$nav.append( '<span class="' + className + '">' + this.slides[ i ].menutitle + '</span>' );
				
				}
				this.$navNext	= $( '<span class="sg-next"></span>' ).appendTo( this.$nav );
				this.$gridWrapper.append( this.$nav.transition( { 'opacity' : 1 }, this.options.fadeSpeed2 ) );
